...
> *Replace the placeholder paths below with your actual screenshot image paths stored in your repository (e.g., `./docs/screenshots/...`).*

## 1. User Dashboard
![User Dashboard](./docs/screenshots/User_dashboard.png)
*Displays available vehicles with dynamic stock badges and purchase triggers.*

### 2. Admin Dashboard
![Admin Dashboard](./docs/screenshots/admin_dashboard.png)
*Admin-only controls to manage, update, and oversee inventory listings.*
...
*Form interface for administrators to add new vehicle models to the inventory.*

### 6. Restock Vehicle Inventory
![Restock Vehicle](./docs/screenshots/restock.png)
*Admin modal to increment existing vehicle stock levels.*

### 7. Vehicle Search & Filter
![Vehicle Search](./docs/screenshots/search.png)
*Real-time vehicle filtering by make, model, category, and price parameters.*
...
* **Test-Driven Development (TDD):** AI models were asked to generate test suites *before* backend endpoints were written. This ensured a strict Red-Green-Refactor development methodology throughout the commit history (e.g., `test(vehicle): add failing vehicle creation tests` followed by `feat(vehicle): implement vehicle creation endpoint`).
